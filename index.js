        ...
        for (var i = 0; i < response.data.data.length; i++) {
          if (
            ...
            var myObject = {
              ...
              statusV: response.data.data[i].shipmentTrackingStatus,
              DLexpiry: new Date(
...
for (const element of result) {
    a=(`</tr><tr><th>VEHICLE NUMBER</th><td>${VehicleNumber}</td></tr><tr><th>Driver Number</th><td>${drivernumber}</td></tr><tr><th>Driver Name</th><td>${DriverName}</td></tr><tr>`);
    // Material.push(`${element.material.name}`)
    // name.push(`${element.consignee.places[0].name}`)

     dataArray.push(a);
    // uuid.push(element.freightUnitLineItemIds)
  
   } 
console.log(dataArray,"etstststst")
     ...
     .then(function (response) {
      ...
      .then(function (response) {
        // console.log(JSON.stringify(response.data));
          console.log(response.status,"gaya check meh INTRANSIT KA ")
      })
